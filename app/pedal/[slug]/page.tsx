...
export default async function PedalDetailPage({
  ...
  */
  const { data: subtype } = await supabase
    .from('subtype')
    .select('*')
    .eq('subtype_id', pedal.subtype_id)
    .single()

  /*
    COMMUNITY
  */
  const { data: userPedals } = await supabase
    .from('user_pedals')
    .select('*')
      ...
      <div className="w-full max-w-md px-5 py-8">
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          <div className="bg-[#faf7f2] rounded-2xl p-4 text-center border border-[#e7dfd3]">
            <p className="text-2xl font-serif text-[#3d3935]">{stats.have}</p>
            <p className="text-xs text-[#6f675d]">Have</p>
          </div>
          <div className="bg-[#faf7f2] rounded-2xl p-4 text-center border border-[#e7dfd3]">
            <p className="text-2xl font-serif text-[#3d3935]">{stats.had}</p>
            <p className="text-xs text-[#6f675d]">Had</p>
          </div>
          <div className="bg-[#faf7f2] rounded-2xl p-4 text-center border border-[#e7dfd3]">
            <p className="text-2xl font-serif text-[#3d3935]">{stats.want}</p>
            <p className="text-xs text-[#6f675d]">Want</p>
          </div>
        </div>
        ...
        <div className="bg-[#faf7f2] rounded-[2rem] p-6 space-y-5 border border-[#e7dfd3]">
          <div>
            <p className="text-xs text-[#8b8175] mb-1">Type</p>
            <p className="text-[#3d3935]">{type?.type || 'Unknown'}</p>
          </div>
          <div>
            <p className="text-xs text-[#8b8175] mb-1">Subtype</p>
            <p className="text-[#3d3935]">{subtype?.subtype || 'Unknown'}</p>
          </div>
          <div>
            <p className="text-xs text-[#8b8175] mb-1">Width</p>
            <p className="text-[#3d3935]">{pedal.w_mm || 'Unknown'} mm</p>
          </div>
          <div>
            <p className="text-xs text-[#8b8175] mb-1">Height</p>
            <p className="text-[#3d3935]">{pedal.h_mm || 'Unknown'} mm</p>
          </div>
          <div>
            <p className="text-xs text-[#8b8175] mb-1">Stereo</p>
            <p className="text-[#3d3935]">{pedal.stereo || 'Unknown'}</p>
          </div>
          <div>
            <p className="text-xs text-[#8b8175] mb-1">Analog / Digital</p>
            <p className="text-[#3d3935]">{pedal.analog_digital || 'Unknown'}</p>
          </div>
          <div>
            <p className="text-xs text-[#8b8175] mb-1">Release Year</p>
            <p className="text-[#3d3935]">{pedal.release_year || 'Unknown'}</p>
          </div>
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-[#f5f1ea]/95 backdrop-blur border-t border-[#e8e1d8]">
          <div className="max-w-md mx-auto flex justify-around py-4 text-sm">
            <Link href="/discover" className="cursor-pointer text-black font-medium">
              Discover
            </Link>
